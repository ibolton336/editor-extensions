...
5. **`vscode-webview-messages`** — `.claude/skills/vscode-webview-messages/`
   - **When**: Adding or changing extension ↔ webview message types, `postMessage`, `MessageTypes`, `useVSCodeMessageHandler`, or shared message contracts.
...
For a **feature-flagged secondary sidebar** variant of the extension, **vscode-secondary-sidebar** and **vscode-webview-view-provider** are especially relevant; use **vscode-feature-flags** to gate the new container/view, and **vscode-webview-messages** / **vscode-extension-state** to keep the new view’s messaging and state consistent with the rest of the extension.
